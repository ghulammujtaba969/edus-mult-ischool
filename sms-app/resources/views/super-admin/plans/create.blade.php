        ...
        <form action="{{ route('super-admin.plans.store') }}" method="POST">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">
                <div>
                    ...
                    <input type="text" name="name" class="form-control-sms @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="e.g. Enterprise Plan" required>
                    @error('name') <div class="text-danger-sms">{{ $message }}</div> @enderror
                </div>
                <div>
                    <label class="form-label-sms">Monthly Price (PKR)</label>
                    <input type="number" step="0.01" name="monthly_price" class="form-control-sms @error('monthly_price') is-invalid @enderror" value="{{ old('monthly_price') }}" placeholder="0.00" required>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:2rem;">
                <div>
                    ...
                    <input type="number" name="max_branches" class="form-control-sms @error('max_branches') is-invalid @enderror" value="{{ old('max_branches', 1) }}" required>
            ...
            <div style="margin-bottom:2rem;">
                <label class="form-label-sms" style="font-size: 1.1rem; margin-bottom: 1rem; display: block; font-weight: 700;">Select Plan Features</label>
                            ...
                            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                                @foreach($features as $key => $label)
                                    <label style="display: flex; gap: 0.5rem; align-items: center; cursor: pointer; font-size: 0.9rem;">
                                        <input type="checkbox" name="features[]" value="{{ $key }}" @checked(is_array(old('features')) && in_array($key, old('features')))>
                                        <span>{{ $label }}</span>
                                    </label>
                                @endforeach
                            </div>
            ...
            <div style="display:flex;gap:1rem; border-top: 1px solid var(--border-color); padding-top: 2rem;">
                <button type="submit" class="btn-primary-sms"><i class="bi bi-save"></i> Save Plan</button>

...
@section('title', 'Edit Class | EduCore SMS')
@section('page_title', 'Edit Class')
@section('breadcrumb', '/ Academic Setup / Classes / Edit')
    ...
    <div class="data-card" style="max-width:600px;margin:0 auto;">
        ...
        <form action="{{ route('admin.classes.update', $class) }}" method="POST">
            ...
            <div style="margin-bottom:1.5rem;">
                ...
                <input class="form-control-sms @error('name') is-invalid @enderror" type="text" id="name" name="name" value="{{ old('name', $class->name) }}" placeholder="e.g. Class 1, Grade 9" required>
            ...
            <div style="margin-bottom:1.5rem;">
                ...
                <select class="filter-select" id="level" name="level">
                    ...
                    @foreach(['primary' => 'Primary', 'middle' => 'Middle', 'secondary' => 'Secondary', 'higher_secondary' => 'Higher Secondary'] as $value => $label)
                        <option value="{{ $value }}" @selected(old('level', $class->level) === $value)>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div style="margin-bottom:2rem;">
                ...
                <input class="form-control-sms @error('order_seq') is-invalid @enderror" type="number" id="order_seq" name="order_seq" value="{{ old('order_seq', $class->order_seq) }}" min="0" required>
